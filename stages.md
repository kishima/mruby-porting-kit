...
**補足**: setjmp／longjmp は選べる実装。ホスト言語に longjmp 相当が無ければ、例外を戻り値で運び、実行ループがフレームの列を歩いて catch ハンドラ表を引く。ホスト関数がブロックを呼び戻すときは入れ子の実行ループを起こし、捕捉されなかった例外は戻り値として外側のループへ返す（vm「大域脱出の二つの流儀」）
 ...
 * class_eval／instance_eval に文字列を渡した後、呼び出し元の def が受け手のクラスに入る → 4.1.0-rc2 の mruby-eval の問題（Proc と共有した環境のターゲットクラスを書き換える）。master で修正済み。本家 rc の挙動に合わせない
 ...
 * 正規表現エンジンを丸ごと移植しようとして手が止まる → エンジンは選べる実装で、処理系の持つ有限オートマトンに置き換えてよい。表面（Regexp、MatchData、String のメソッド、$~）は本家から移植する。ただし後方参照・先読み・後読み・アトミックグループ・絶対最大量指定子・部分式呼び出し・条件・不在演算子は有限オートマトンに無く、言語仕様の差になる。コンパイル時に構文名を入れた RegexpError で断り、本家テストの該当件数を意図した差異として数える
 ...
 * $~ をグローバル変数の表に置くと呼び出し元に漏れる → 4.1.0-rc2 の $~ はメソッドのスコープが持つ特別変数。ブロックは定義元と共有し、C のフレームは素通しで下の Ruby フレームに書き、Fiber は自分のものを持つ。$1 や $& はコンパイラが $~ の読み方に落とす。スコープの環境（REnv）に容れ物を置き、容れ物は最初の非 nil 書き込みまで作らない（読みは作らない）
 ...
 * Fiber とタスクを混ぜると壊れる（本家の README は「混ぜるな」） → 本家の理由は 2 つとも実装の帰結。(1) コンテキストからタスクを逆引きするのがポインタ演算なので、Fiber の中で Task.current や sleep を呼ぶと無関係な番地を読む。(2) タイムスライスが Fiber の中で切れると Fiber のコンテキストで実行ループを抜け、Fiber が宙に浮く。逆引きを「走っているタスクの文脈と一致するか」にし、Fiber の中では切り替えを保留する（ネイティブ境界と同じ規則。Fiber は resume というネイティブを通って入る）と共存できる。代償は 2 つで、Fiber の中にいる間そのタスクは横取りされない、Fiber の中からは Task.current／sleep／Task.pass が「タスクではない」扱いになる
